...
function Dashboardselector() {
  ...
  const DashboardButton = ({ role, label, className }) => (
    <button
  ...
  // ✅ Render dashboard buttons conditionally
  const renderDashboardButtons = () => {
    if (!isAuthenticated) {
      // Show all by default when not logged in
      return (
        <>
      ...
      <header className="bg-blue-400 shadow-md relative overflow-hidden">
        ...
        {menuOpen && (
          <div className="md:hidden bg-orange-600 px-4 pb-4 space-y-2">
            ...
            {/* ✅ Role-based dashboard buttons in mobile menu */}
            {renderDashboardButtons()}
          </div>
        )}
        {/* Hero Section */}
        {/* (rest of your hero section unchanged) */}
